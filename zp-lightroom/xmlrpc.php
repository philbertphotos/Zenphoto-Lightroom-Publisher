...
function authorize($args)
{
	...
	//debugLog("uobject: " .$userobj);
	//$userobj1 = $_zp_authority->getAnAdmin(array('<code>user</code>=' => $args['loginUsername'], '<code>pass</code>=' => $hash, '<code>valid</code>=' => 1));
	/**debuglog('checkLogon: '.checkLogon($args['loginUsername'], $args['loginPassword']));
	...
	debugLog("local Hash: " .$userobj->getPass());
...
function changeAlbum($args)
{
	...
	if ($newfolder && $albumobject->name != $newfolder) {
	 ...
	 //$query = query('INSERT INTO '.prefix('plugin_storage').' (type,aux,data) VALUES ("albumpassword", '.$album->getID().', '.$album->getPassword().')');
